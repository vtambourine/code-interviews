...
initalProducts.trim().split('\n').forEach(line => {
  ...
  const value = parseInt(productData[1]);
  const amount = parseInt(productData[2]);
...
initalCoins.trim().split('\n').forEach(line => {
  const coinData = line.split(' ');
  const name = coinData[0];
  const amount = parseInt(coinData[1]);
...
client.on('line', function (line) {
  ...
  if (input) {
    ...
    switch (cmd) {
      case 'list':
        console.log('We have:');
        for (let key in Machine.productsStore) {
          console.log(
            '%s by %s - %s left',
            key,
            pounds(Machine.productValues[key]),
            Machine.productsStore[key]
          );
        }
...
function processLoad(loads) {
  ...
  try {
    ...
    for (let i = 0; i < loads.length; i += 2) {
      let load = loads[i].trim();
      let amount = parseInt(loads[i + 1], 10);
...
function processChange() {
  ...
  if (creditCoins.length) {
    creditCoins.forEach(coin => {
      let value = Machine.coinValues[coin];
      credit -= value;
      Machine.coinsStore[coin]--;
      newChange.push(coin);
    });
  ...
  Machine.coinsByValue.forEach(coin => {
    let value = Machine.coinValues[coin];
    while (credit >= value) {
